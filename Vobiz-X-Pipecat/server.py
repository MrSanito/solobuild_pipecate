...
<Response>
    ...
    try:
        ...
        if env == "production":
            ...
            agent_name = os.getenv("AGENT_NAME")
            org_name = os.getenv("ORGANIZATION_NAME")
            final_ws_url = f"wss://{region}.api.pipecat.daily.co/ws/generic?serviceHost={agent_name}.{org_name}"
            if parsed_body_data:
                body_json = json.dumps(parsed_body_data)
                body_encoded = base64.b64encode(body_json.encode("utf-8")).decode("utf-8")
                final_ws_url = f"{final_ws_url}&body={body_encoded}"
        else:
...
async def handle_vobiz_websocket(
    ...
    try:
        ...
        # Extract gemini_api_key from body_data if present
        gemini_api_key = body_data.get("gemini_api_key") if body_data else None
        # We pass call_id if we have it, but we let stream_id be None so bot/transport can find it from the stream
        
        await bot(runner_args, call_id=call_uuid, stream_id=stream_id, gemini_api_key=gemini_api_key)
