...
async def run_bot(transport: BaseTransport, handle_sigint: bool, name: str = "", number: str = "", lead_id: str = ""):
    ...
    # (You can enable this to see how it works)
    ...
    async def on_client_connected(transport, client):
        logger.info(f"Starting outbound call conversation with name: {name}")
        greeting_name = name.strip()
        if greeting_name:
            phrase = f"Say this exact phrase: 'Hello {greeting_name}, mera naam Kabir hai Showtime Events se. Hamare upcoming Global Trade aur Logistics Expo ke regarding baat karni thi. Kya main do minute baat kar sakta hu?'"
        else:
            phrase = "Say this exact phrase: 'Hello, mera naam Kabir hai Showtime Events se. Hamare upcoming Global Trade aur Logistics Expo ke regarding baat karni thi. Kya main do minute baat kar sakta hu?'"
...
async def bot(runner_args: RunnerArguments, call_id: str = None, stream_id: str = None):
    ...
    if hasattr(runner_args, "websocket") and runner_args.websocket:
        name = runner_args.websocket.query_params.get("name", "")
        number = runner_args.websocket.query_params.get("number", "")
        lead_id = runner_args.websocket.query_params.get("lead_id", "")
        ...
        logger.info("Initializing bot in Daily WebRTC room mode")
    ...
    await run_bot(transport, handle_sigint, name=name, number=number, lead_id=lead_id)